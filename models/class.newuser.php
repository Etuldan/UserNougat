...
class User 
{
	...
	public function userNougatAddUser()
	{
		global $mysqli,$emailActivation,$websiteUrl,$db_table_prefix,$firstUser;
		...
		if($this->status)
		{
			...
			if(!$this->mail_failure)
			{
				...
				//Reset first User on db configuration
				if($firstUser == 1) {
				...
				foreach (glob("plugins/*.php") as $filename) {
					include($filename);
					$module->createUser($inserted_id,$this->username, $this->displayname, $this->clean_password, $this->clean_email);
					if($firstUser == 1) {
						$module->setAsAdmin($inserted_id, $this->username);
					}
				}
			}
		}
